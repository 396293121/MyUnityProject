...
namespace Fungus
{
    ...
    public class SayDialog : MonoBehaviour
    {
        ...
        public void SetButtonClickEvent(){
            ...
            {
                // 通过查找当前活动的 Flowchart 和正在执行的 Block 来继续对话
                var activeFlowcharts = Flowchart.CachedFlowcharts;
                foreach (var flowchart in activeFlowcharts)
                {
                    if (flowchart != null && flowchart.isActiveAndEnabled)
                    {
                        Debug.Log($"[SayDialog] 找到活动的Flowchart: {flowchart.name}");
                        // 查找当前正在执行的 Block
                        var blocks = flowchart.GetComponents<Block>();
                        int i = 0;
                        foreach (var block in blocks)
                        {
                            Debug.Log($"[SayDialog] 正在执行的Block: {block.name}");
                            if (block.IsExecuting() && block.ActiveCommand != null)
                            {
                                Debug.Log($"[SayDialog] 正在执行的命令: {block.ActiveCommand.name}");
                                // 如果找到正在执行的 Say 命令，调用其 Continue 方法
                                if (block.ActiveCommand is Say)
                                {
                ...
                Debug.Log("daan1");
                                    ...
                                    Debug.Log($"[SayDialog] 找到Say命令，调用Continue方法");
                                    block.ActiveCommand.Continue();
                                    return;
                                }
                            }
                            i++;
        ...
        protected virtual void Start()
        {
            ...
            Debug.Log($"[SayDialog] {name} 初始化完成 - Alpha: {canvasGroup.alpha}, GameObject激活: {gameObject.activeInHierarchy}");
        }
  ...
  public static void ForceCloseDialog()
    {
        // 获取所有活动的SayDialog实例
        SayDialog[] activeDialogs = GameObject.FindObjectsOfType<SayDialog>();
        
        foreach (SayDialog dialog in activeDialogs)
        {
            // 停止所有关联的Flowchart
            var flowchart = Flowchart.CachedFlowcharts;

            if (flowchart != null)
            {
                foreach (var fc in flowchart)
                {
                    fc.StopAllBlocks();
            ...
            // 销毁对话历史
            if (dialog.gameObject != null)
            {
                // 恶魔城风格：添加关闭特效
                Debug.Log("对话关闭"+dialog);
                // 延迟销毁确保完全关闭
