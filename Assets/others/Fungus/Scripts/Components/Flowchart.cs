...
{
    /// <summary>
    /// 流程图编程语言的可视化脚本控制器 | Visual scripting controller for the Flowchart programming language.
    /// 流程图对象可以通过流程图编辑器窗口进行可视化编辑 | Flowchart objects may be edited visually using the Flowchart editor window.
    /// </summary>
    [ExecuteInEditMode]
    public class Flowchart : MonoBehaviour, ISubstitutionHandler
    {
        ...
        [TextArea(3, 5)]
        [LabelText("描述")]
        [Tooltip("描述文本显示在流程图编辑器窗口中 | Description text displayed in the Flowchart editor window")]
        [SerializeField] protected string description = "";

        [Range(0f, 5f)]
        [LabelText("步骤暂停")]
        [Tooltip("在每个执行步骤后添加暂停，使程序流程更容易可视化。仅编辑器有效，在平台构建中无效果 | Adds a pause after each execution step to make it easier to visualise program flow. Editor only, has no effect in platform builds.")]
        [SerializeField] protected float stepPause = 0f;

        [LabelText("命令颜色")]
        [Tooltip("在Fungus编辑器窗口中显示命令列表时使用命令颜色 | Use command color when displaying the command list in the Fungus Editor window")]
        [SerializeField] protected bool colorCommands = true;

        [LabelText("隐藏组件")]
        [Tooltip("在检查器中隐藏流程图块和命令组件。取消选择以检查构成流程图的块和命令组件 | Hides the Flowchart block and command components in the inspector. Deselect to inspect the block and command components that make up the Flowchart.")]
        [SerializeField] protected bool hideComponents = true;

        [LabelText("保存选择")]
        [Tooltip("保存场景时保存选定的块和命令。如果只更改了活动选择，有助于避免版本控制冲突 | Saves the selected block and commands when saving the scene. Helps avoid version control conflicts if you've only changed the active selection.")]
        [SerializeField] protected bool saveSelection = true;

        [LabelText("本地化ID")]
        [Tooltip("此流程图在本地化字符串键中的唯一标识符。如果未指定id，则将使用流程图对象的名称 | Unique identifier for this flowchart in localized string keys. If no id is specified then the name of the Flowchart object will be used.")]
        [SerializeField] protected string localizationId = "";

        [LabelText("显示行号")]
        [Tooltip("在块检查器的命令列表中显示行号 | Display line numbers in the command list in the Block inspector.")]
        [SerializeField] protected bool showLineNumbers = false;

        [LabelText("隐藏命令")]
        [Tooltip("在添加命令菜单中隐藏的命令列表。使用此选项限制编辑流程图时可用的命令集 | List of commands to hide in the Add Command menu. Use this to restrict the set of commands available when editing a Flowchart.")]
        [SerializeField] protected List<string> hideCommands = new List<string>();

        [LabelText("Lua环境")]
        [Tooltip("此流程图中所有执行Lua命令默认使用的Lua环境 | Lua Environment to be used by default for all Execute Lua commands in this Flowchart")]
        [SerializeField] protected LuaEnvironment luaEnvironment;

        [LabelText("Lua绑定名称")]
        [Tooltip("ExecuteLua命令在执行前添加一个绑定到流程图的全局Lua变量，使用此名称 | The ExecuteLua command adds a global Lua variable with this name bound to the flowchart prior to executing.")]
        ...
        /// <summary>
        /// 场景中流程图对象的缓存列表，用于快速查找 | Cached list of flowchart objects in the scene for fast lookup.
        /// </summary>
        public static List<Flowchart> CachedFlowcharts { get { return cachedFlowcharts; } }

        /// <summary>
        /// 向当前场景中的所有流程图对象发送消息 | Sends a message to all Flowchart objects in the current scene.
        /// 任何具有匹配MessageReceived事件处理程序的块都将开始执行 | Any block with a matching MessageReceived event handler will start executing.
        /// </summary>
        ...
        /// <summary>
        /// 流程图编辑器中当前选择的命令 | Currently selected command in the Flowchart editor.
        /// </summary>
        public virtual List<Command> SelectedCommands { get { return selectedCommands; } }

        /// <summary>
        /// 流程图可以访问的变量列表 | The list of variables that can be accessed by the Flowchart.
        /// </summary>
        ...
        /// <summary>
        /// 描述文本显示在流程图编辑器窗口中 | Description text displayed in the Flowchart editor window
        /// </summary>
        public virtual string Description { get { return description; } }

        /// <summary>
        /// 在编辑器中减慢执行速度，使程序流程更容易可视化 | Slow down execution in the editor to make it easier to visualise program flow.
        /// </summary>
        public virtual float StepPause { get { return stepPause; } }

        /// <summary>
        /// 在检查器中显示命令列表时使用命令颜色 | Use command color when displaying the command list in the inspector.
        /// </summary>
        public virtual bool ColorCommands { get { return colorCommands; } }

        /// <summary>
        /// 保存场景时保存选定的块和命令。如果只更改了活动选择，有助于避免版本控制冲突 | Saves the selected block and commands when saving the scene. Helps avoid version control conflicts if you've only changed the active selection.
        /// </summary>
        public virtual bool SaveSelection { get { return saveSelection; } }

        /// <summary>
        /// 在本地化字符串键中标识此流程图的唯一标识符 | Unique identifier for identifying this flowchart in localized string keys.
        /// </summary>
        public virtual string LocalizationId { get { return localizationId; } }

        /// <summary>
        /// 在块检查器的命令列表中显示行号 | Display line numbers in the command list in the Block inspector.
        /// </summary>
        public virtual bool ShowLineNumbers { get { return showLineNumbers; } }

        /// <summary>
        /// 此流程图中所有执行Lua命令默认使用的Lua环境 | Lua Environment to be used by default for all Execute Lua commands in this Flowchart.
        /// </summary>
        public virtual LuaEnvironment LuaEnv { get { return luaEnvironment; } }

        /// <summary>
        /// ExecuteLua命令在执行前添加一个绑定到流程图的全局Lua变量，使用此名称 | The ExecuteLua command adds a global Lua variable with this name bound to the flowchart prior to executing.
        /// </summary>
        public virtual string LuaBindingName { get { return luaBindingName; } }

        /// <summary>
        /// 流程图中所有块的中心位置 | Position in the center of all blocks in the flowchart.
        /// </summary>
        public virtual Vector2 CenterPosition { set; get; }

        /// <summary>
        /// 跟踪流程图版本的变量，以便组件可以更新到新版本 | Variable to track flowchart's version so components can update to new versions.
        /// </summary>
        public int Version { set { version = value; } }

        /// <summary>
        /// 如果流程图游戏对象处于活动状态，则返回true | Returns true if the Flowchart gameobject is active.
        /// </summary>
        ...
        /// <summary>
        /// 返回流程图游戏对象的名称 | Returns the Flowchart gameobject name.
        /// </summary>
        ...
        /// <summary>
        /// 返回要分配给新流程图项目的下一个id | Returns the next id to assign to a new flowchart item.
        /// 项目id单调递增，因此保证在流程图内是唯一的 | Item ids increase monotically so they are guaranteed to
