...
#### 角色系统（已迁移到MapSystemConfig）
```csharp
// 注意：角色配置已迁移到MapSystemConfig中
// 在UnifiedSceneConfig中只保留玩家生成位置配置
[TabGroup("基础配置", "玩家生成")]
public class PlayerSpawnSystem
{
    public Vector3 playerSpawnPosition;      // 玩家生成点位置
}
```
...
#### 玩家生成配置（角色配置已迁移）
```
玩家生成点位置：设置玩家生成的世界坐标位置

注意：角色预制体配置已迁移到MapSystemConfig中，包括：
  - 支持的角色类型配置
  - 角色预制体列表
  - 默认角色类型设置
  
请在MapSystemConfig中进行角色相关配置。
```
