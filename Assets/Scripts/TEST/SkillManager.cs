...
public class SkillManager : MonoBehaviour
{
    ...
    [LabelText("可用技能列表")]
    public List<skillDataConfig> availableSkills = new List<skillDataConfig>();
    ...
    private Dictionary<skillDataConfig, float> skillCooldowns = new Dictionary<skillDataConfig, float>();
    ...
    public bool UseSkill(skillDataConfig skill)
    {
        // 检查技能是否可使用
    ...
    void ExecuteSkill(skillDataConfig skill)
    {
        // 播放技能动画
    ...
    /// 延迟执行技能效果，配合动画时机
    /// </summary>
    public void OnDelayedSkillEffect(skillDataConfig skill)
    {
        
        // 委托给skillDataConfig执行具体的技能效果
        skill.ExecuteSkillEffect(gameObject, skillSpawnPoint.position, skillSpawnPoint);
    }
    
    void UpdateCooldowns()
    {
        var keys = new List<skillDataConfig>(skillCooldowns.Keys);
    ...
    public float GetSkillCooldown(skillDataConfig skill)
    {
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        foreach (var skill in availableSkills)
        {
            if (skill.showHitbox)
            {
                skillDataConfig.SkillGizmoData gizmoData = skill.GetSkillGizmoData();
                ...
                switch (gizmoData.skillType)
                {
                    ...
                    case SkillTypeTest.SingleTargetBox:
                        {
                            // 根据角色的localScale.x判断朝向，与角色脚本保持一致
                            Vector3 forward = transform.localScale.x > 0 ? Vector3.right : Vector3.left;
                            DrawDirectionalBoxGizmo(drawPosition, forward, 
                    ...
                    case SkillTypeTest.AreaOfEffect:
                        ...
                        {
                            // 方向性矩形AOE
                            DrawDirectionalAOEBoxGizmo(drawPosition, 
                ...
                {
                    case SkillTypeTest.SingleTargetBox:
                        labelText = $"F:{gizmoData.boxForward} B:{gizmoData.boxBackward} L:{gizmoData.boxLeft} R:{gizmoData.boxRight}";
                        break;
                    case SkillTypeTest.AreaOfEffect:
                        if (gizmoData.isCircularAOE)
                            labelText = $"R:{gizmoData.range}";
                        else
                            labelText = $"U:{gizmoData.aoeUp} D:{gizmoData.aoeDown} L:{gizmoData.aoeLeft} R:{gizmoData.aoeRight}";
                        break;
                    default:
                        labelText = $"R:{gizmoData.range}";
                        break;
                }
                UnityEditor.Handles.Label(drawPosition + Vector3.up * 0.5f, labelText);
            }
        }
    }
        ...
        float forwardDist, float backwardDist, float leftDist, float rightDist)
    {
        ...
        // 计算矩形的四个角点
        Vector3 perpendicular = new Vector3(-forward.y, forward.x, 0); // 垂直于forward的方向
        Vector3 halfLength = forward * (totalLength / 2f);
        Vector3 halfWidth = perpendicular * (totalWidth / 2f);
        
        Vector3[] corners = new Vector3[4]
        {
            center + halfLength + halfWidth,  // 前右
            center + halfLength - halfWidth,  // 前左
            center - halfLength - halfWidth,  // 后左
            center - halfLength + halfWidth   // 后右
        };
        ...
        float upDist, float downDist, float leftDist, float rightDist)
    {
        ...
        // 计算矩形的四个角点（AOE矩形与世界坐标轴对齐）
        Vector3 halfWidth = Vector3.right * (totalWidth / 2f);
        Vector3 halfHeight = Vector3.up * (totalHeight / 2f);
        
        Vector3[] corners = new Vector3[4]
        {
            center + halfWidth + halfHeight,  // 右上
            center + halfWidth - halfHeight,  // 右下
            center - halfWidth - halfHeight,  // 左下
            center - halfWidth + halfHeight   // 左上
        };
