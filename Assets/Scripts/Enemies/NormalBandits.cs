...
/// <summary>
/// 普通山贼敌人类 - 具有冲撞攻击能力的近战敌人
/// 继承自重构后的Enemy基类，采用SimpleEnemy的状态机设计
/// 特色功能：冲锋攻击、醉酒状态、撞墙眩晕
/// 性能优化：屏幕外减少更新频率
/// </summary>
[ShowOdinSerializedPropertiesInInspector]
public class NormalBandits : Enemy
{
    [TabGroup("普通山贼配置", "技能触发")]
    [FoldoutGroup("普通山贼配置/技能触发/技能条件", expanded: true)]
    [LabelText("醉酒血量阈值")]
    [ReadOnly]
    [ShowInInspector]
    [InfoBox("血量低于此百分比时触发醉酒技能")]
    public float enrageSkillTriggerThreshold = 0.3f;
    [FoldoutGroup("普通山贼配置/技能触发/技能条件")]
    [LabelText("眩晕持续时间")]
    [ReadOnly]
    [ShowInInspector]
    private float stunDuration;
    [TabGroup("普通山贼配置", "状态监控")]
    [FoldoutGroup("普通山贼配置/状态监控/技能状态", expanded: true)]
    [LabelText("是否已触发醉酒")]
    [ReadOnly]
    [ShowInInspector]
    ...
    private NormalBanditsConfig NormalBanditsConfig;
    // 普通山贼眩晕状态动画
    ...
    public override void Awake()
    {
        ...
        {
            Debug.Log($"[NormalBandits] 普通山贼创建完成 - 生命值: {currentHealth}/{maxHealth}, 攻击力: {attackDamage}");
        }
    }
    /// <summary>
    /// 调试可视化 - 普通山贼专用
    ...
    protected override void InitializeFromSystemConfig()
    {
        ...
        NormalBanditsConfig ??= FindObjectOfType<NormalBanditsConfig>();
        if (NormalBanditsConfig != null)
        {

            // 初始化技能触发条件
            enrageSkillTriggerThreshold = NormalBanditsConfig.enrageHealthThreshold;
            stunDuration = NormalBanditsConfig.stunDuration;
            if (GameManager.Instance != null && GameManager.Instance.debugMode)
            {
                Debug.Log($"[NormalBandits] 普通山贼配置初始化完成 , 醉酒触发阈值: {enrageSkillTriggerThreshold}");
            }
        }
    }
    ...
    protected override void Update()
    {
        ...
        // 只处理普通山贼特有的逻辑
        UpdateNormalBanditsSpecificLogic();
    }

    /// <summary>
    /// 更新普通山贼特有逻辑
    /// </summary>
    private void UpdateNormalBanditsSpecificLogic()
    {
        // 检查技能触发条件
     //   CheckSkillTriggers();
    }
    ...
    private void TriggerEnrageSkill()
    {
        ...
        if (skillComponent.skillDataList[0] != null && skillComponent.TryUseSkill(0)) // 醉酒技能在索引0

        {
            ...
            if (GameManager.Instance != null && GameManager.Instance.debugMode)
            {
                Debug.Log($"[NormalBandits] 触发醉酒技能");
            }
        }
        else
        {
            Debug.LogWarning($"[NormalBandits] 醉酒技能触发失败");
        }
    }
...
//             transform.localScale.z);        
        ...
        {
            Debug.LogWarning($"[NormalBandits] 冲锋技能触发失败");
        }
    }
    ...
    public override void ExecutePatrol()
    {
        ...
        // 采用简化的巡逻逻辑
        Vector2 patrolDirection = GetNormalBanditsPatrolDirection();
    ...
    /// 获取普通山贼巡逻方向 - 采用稳定巡逻机制
    /// </summary>
    private Vector2 GetNormalBanditsPatrolDirection()
    {
        // 使用更长的巡逻周期，减少频繁切换
