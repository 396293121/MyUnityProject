...
//正在攻击用于调试框
    ...
    public virtual void TakeDamage(int damage, Vector2 hitPoint, Character attacker)
    {
        // 调用原有的TakeDamage方法，默认为物理伤害
        TakeDamage(damage, false);
        
        // 可以在这里添加额外的逻辑，如击中特效、攻击者信息处理等
        if (GameManager.Instance != null && GameManager.Instance.debugMode)
        {
            Debug.Log($"[{gameObject.name}] 被 {attacker?.name ?? "未知攻击者"} 在 {hitPoint} 位置攻击");
        }
    }
