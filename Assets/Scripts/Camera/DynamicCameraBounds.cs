...
{
    #region 边界检测设置
    [FoldoutGroup("边界检测设置", expanded: true)]
    [LabelText("启动时自动检测")]
    [InfoBox("勾选后，脚本会在Start()时自动检测并设置相机边界")]
    ...
    [LabelText("Tilemap数组")]
    [InfoBox("要检测边界的Tilemap对象数组，可以包含多个地图层")]
    ...
    public void DetectAndSetBounds()
    {
        CalculateBounds();
        ApplyBoundsToCamera();
    }
    ...
    /// 手动设置Tilemap数组
    /// </summary>
    /// <param name="newTilemaps">新的Tilemap数组</param>
    [FoldoutGroup("操作按钮")]
    [Button("设置Tilemap数组")]
    public void SetTilemaps(Tilemap[] newTilemaps)
    {
    ...
    /// </summary>
    [FoldoutGroup("操作按钮")]
    [Button("自动查找Tilemap", ButtonSizes.Medium)]
    [InfoBox("自动在场景中查找所有Tilemap组件")]
    public void AutoFindTilemaps()
    {
        tilemaps = FindObjectsOfType<Tilemap>();
        if (showDebugInfo)
            Debug.Log($"在场景中找到 {tilemaps.Length} 个Tilemap");
    }
